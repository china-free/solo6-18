...
import { ScanReport, ScanBaselineDiff, OutdatedDependency, SecurityVulnerability, VersionConflict } from './types.js';
...
export class ReportGenerator {
  ...
  private renderOutdatedDiffMarkdown(lines: string[], diff: ScanBaselineDiff): void {
  ...
  private renderVulnDiffMarkdown(lines: string[], diff: ScanBaselineDiff): void {
  ...
  private renderConflictDiffMarkdown(lines: string[], diff: ScanBaselineDiff): void {
  ...
  diffToConsole(diff: ScanBaselineDiff): string {
    ...
    const trendConfig: Record<string, { label: string; color: (s: string) => string }> = {
    ...
    trendTable.push(
      ['过时依赖', String(diff.summary.outdatedAdded), String(diff.summary.outdatedRemoved), String(diff.summary.outdatedUnchanged)],
      ['安全告警', String(diff.summary.vulnAdded), String(diff.summary.vulnRemoved), String(diff.summary.vulnUnchanged)],
      ['版本冲突', String(diff.summary.conflictAdded), String(diff.summary.conflictRemoved), String(diff.summary.conflictUnchanged)]
    );
