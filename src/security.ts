...
const BUILTIN_VULNERABILITIES: VulnerabilityDBEntry[] = [
  {
    name: 'lodash',
    severity: 'high',
...
export class SecurityChecker {
  ...
  loadVulnerabilitiesFromFile(filePath: string): void {
    try {
      ...
      const vulns: VulnerabilityDBEntry[] = JSON.parse(content);
    ...
    for (const project of projects) {
      for (const type of depTypes) {
        ...
        for (const [name, versionRange] of Object.entries(deps)) {
          ...
          if (vulnEntries.length === 0) continue;
  ...
  private isVulnerable(version: string, vulnerableRange: string): boolean {
    try {
